...
if (isset($_POST['send'])) { 
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx','png','jpg','doc','ppt','pptx','txt','xlsx','image/*','mp4'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO users (username,firstname,surname,ticketname,projectname,coursename,file, filesize, downloads) VALUES ('$username','$firstname','$surname','$titleofwork','$projectname','$coursename','$filename', $size, 0)";
            if (mysqli_query($conn, $sql)) {
                echo "Work submitted ";
                return Redirect::to('submitwork');            }
        } else {
